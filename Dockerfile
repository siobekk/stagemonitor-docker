...
# Elastic Search
run    cd ~ && wget https://download.elasticsearch.org/elasticsearch/elasticsearch/elasticsearch-1.7.4.deb
run    cd ~ && dpkg -i elasticsearch-1.7.4.deb && rm elasticsearch-1.7.4.deb
...
run cd /tmp && wget http://grafanarel.s3.amazonaws.com/grafana-1.9.1.tar.gz &&\
	...
	mv /tmp/grafana-1.9.1 /www/data/grafana
run cd /tmp && wget https://download.elasticsearch.org/kibana/kibana/kibana-3.1.3.tar.gz &&\
	tar xzvf kibana-3.1.3.tar.gz && rm kibana-3.1.3.tar.gz &&\
	mv /tmp/kibana-3.1.3 /www/data/kibana
